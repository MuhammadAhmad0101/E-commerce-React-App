...
function Card() {
      ...
      return filterProduct.length > 0 ? (
            filterProduct.map((item, index) => {
                  return (
                        <div key={index} className="w-full rounded-3xl h-full p-4 flex items-center gap-2 bg-[#FFFFFF]">
                              <div className="rounded-2xl overflow-hidden w-[44%] h-full flex justify-center items-center">
                                    <img className="w-full h-full  object-cover" src={item.image} loading="lazy" alt="Loadthumbnail......" />
                              </div>
                              <div className="bg-[#F3EFEE]/50 backdrop-blur-md w-[64%] h-[90%] flex items-center flex-col gap-3 rounded-2xl  p-3">
                                    <div className="w-full relative mt-2">
                                          ...
                                          <p className="font-Satoshi font-xs mt-5 leading-none bg-[#d4d4e9] text-[#675d98] rounded-full p-2 px-3  text-center text-nowrap ">{item.category.toLowerCase()}</p>
                                    </div>
                                    <div className="w-full flex justify-between items-center mt-3">
                                          <h1 className="text-[#3D3857] text-xl font-Poppins font-bold">${item.price}</h1>
